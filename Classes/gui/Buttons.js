class Buttons{
    constructor(zoom){
        let selectBallButton = document.getElementById('selectBall');
        let addBallButton = document.getElementById('addBall');
        let moveCameraButton = document.getElementById('moveCameraButton');
        this.zoom = zoom;
        moveCameraButton.addEventListener('click', this.selectGrabbing.bind(this));
        selectBallButton.addEventListener('click', function () {
           clickGUI.setUpClick();
        });
        this.grabLastPoint = new Pos(0,0);
    }
    selectGrabbing(){
        clickGUI.removeClick();
        console.log("NEW");
